...
const generate_proof_helper = (level: any, index: number, proof: any, cachedLevels: {}): any => {
  if (level.length === 1) {
    return proof;
  }
  ...
  if (!cachedLevels[level]) {
    cachedLevels[level] = get_next_level(level);
  }
  let index_parent = 0;
  ...
  return generate_proof_helper(cachedLevels[level], index_parent, proof, cachedLevels);
};

export const generate_merkle_proof = (values: any[], index: any, cachedLevels: {}): any => {
  return generate_proof_helper(values, index, [], cachedLevels);
};
